...
public class ArrivescanActivity extends Activity {
	...
	TextView verify,send_number_count,upload_daojian_flag;// 验证
	TextView nolocal;// 未锁
	Button fanhui;// 返回
	Button delect;// 删除
	Button queding;// 确定
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<DaoJianSQLite> daojianlist =getDaojianlist(current_all_number, 3);
...
//					check_site_code(pretname.getText().toString());
		...
		fanhui.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				showDialog(ArrivescanActivity.this,"确认退出当前界面？");
			}
		});
	...
	public void send_flag_fajian(){
		...
		paramsmap.put("scanDataList", jsont.getDaoJianDataList(getDaojianlist((int) saveShard.getCount("到件"), sending_count_n)));
	...
	public void saveDabaseDaoJian(){
		WeakUPScreen.onScan(ArrivescanActivity.this);
			...
			if(timeQuery.setDaojianCursor(number_value)){
				...
				adapter.setmMessageItems(timeQuery.daoJianSQLite);
				adapter.notifyDataSetChanged();
				play_beep_sound.playBeepSoundAndVibrate(1);
	...
	public void mGetSaveDabaseSQ(boolean falg,int current_all_number){
		ArrayList<DaoJianSQLite> list = getDaojianlist(current_all_number, 3);
		if(!falg){
			daojianlist.clear();
		}
